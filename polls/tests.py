...
class QuestionIndevViewTests(TestCase):
  def test_no_question(self):
    """
    If no questions exist, an approprieate message displayed
    """
  ...
  def test_future_question(self):
    """
    Question from the future aren't displayed in the index page
    """
  ...
  def test_two_past_question(self):
    ...
    question_2 = create_question(question_text='Past question 2', days=-2)
    response = self.client.get(reverse('polls:index'))
    self.assertQuerysetEqual(
      response.context['latest_question_list'],
